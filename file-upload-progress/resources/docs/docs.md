...
The `home-routes` handler in the `file-upload-progress.routes.home` namespace
...
The `home-page` function will in turn call the `file-upload-progress.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `file-upload-progress.handler` namespace:
...
Request middleware functions are located under the `file-upload-progress.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `file-upload-progress.dev-middleware` namespace found in
